...
export class ActividadesService {
  constructor(
  @InjectRepository(Actividad) private actividadesRepository: Repository<Actividad>, private usuariosService: UsuarioService,){
  ...
  findpendiente(): Promise<Actividad[]> {
    const pendiente  = this.actividadesRepository.find({ where: { estado: EstadoActividadEnum.pendiente } });
    return pendiente
  } 
  finidrep(userId:number): Promise<Actividad[]>
   {        const repartidor  =this.actividadesRepository.find({ where: { id_usuario_actual: userId} });
        return repartidor
  } 
  ...
  async crearActividad(actividadesdto: ActividadesDto, usuario:Usuario){
    ...
    actividad.usuarioActual = await this.usuariosService.findOneById(actividadesdto.id_usuario_actual);
  ...
  async update(id: number, actividadesdto: ActividadesDto, usuario:Usuario): Promise<Actividad> {
    const actividad: Actividad = this.actividadesRepository.create();
    actividad.descripcion = actividadesdto.descripcion;
  ...
  // Método admin para actualizar una actividad existente 
  async updaterep(id: number, actividadesdto: ActividadesDto, usuario:Usuario): Promise<Actividad> {
    const actividad: Actividad = this.actividadesRepository.create();
  ...
  async remove(id: number): Promise<void> {
    ...
    const actividadelim = await this.actividadesRepository.findOne({ where: { id } });
    if (!actividadelim) {
